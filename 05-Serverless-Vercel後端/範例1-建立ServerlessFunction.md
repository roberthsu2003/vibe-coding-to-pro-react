...
> ⚠️ **這一步非常重要！**  
> `.env` 裡面有你的真實 API Key，絕對不能上傳到 GitHub。  
...
回答完畢後，它背後幫我們做了兩件非常關鍵的事：
1. **本機綁定**：在專案底下建立一個 `.vercel` 隱藏資料夾，記錄了這個專案的 ID 綁定到你的帳號。
2. **雲端建立 App（重要！）**：其實就在你回答完問題的那一刻，Vercel **已經在你的雲端帳號裡建立了一個同名的 App（專案）！** 它目前只是一個空殼，但已經準備好在未來接收我們的程式碼上傳了。
